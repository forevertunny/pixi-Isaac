...
Slider.prototype.initialize = function ()
{
    ...
    handleDrag.onDragStart = function (event)
    {
        ...
        this.maxPosition = self.vertical ? self._height - self.handle._height : self._width - self.handle._width;
    };

    handleDrag.onDragMove = function (event, offset)
    {

        self._amt = !this.maxPosition ? 0 : Math.max(0, Math.min(1, startValue + ((self.vertical ? offset.y : offset.x) / this.maxPosition)));
